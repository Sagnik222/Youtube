...
// Coordinates the scraping, script generation, simulated uploads, analytics tracking, and daily reports compilation.

import fs from 'fs';
import path from 'path';
import { getCuratedProducts, getProductByName } from './scraper.js';
import { generateShortsScript, formatScriptToMarkdown } from './scriptwriter.js';
import { uploadVideoMock, checkCredentials } from './youtube-uploader.js';
...
// Config: run daemon tick interval (defaults to 15 seconds for prototype demonstration)
...
function queueNewTool(data) {
  ...
  if (nextTool) {
    ...
    console.log(`[Daemon] Queued new tool to database: ${nextTool.name}`);
  }
}
...
function compileDashboardReport(data) {
  ...
  let md = `# 📊 YouTube Automation Daemon Active: Daily Performance Brief\n`;
  md += `*Last updated: ${new Date().toLocaleString()}* | **Status:** 🟢 Running\n\n`;
  ...
  md += `- **Next Scan / Generation:** Daemon is active and checking for new ProductHunt trends.\n`;
  md += `- **API Auth Setup:** The daemon is in mock-mode. To trigger real YouTube uploads, drop your \`client_secret.json\` into the \`.credentials/\` directory.\n`;
  md += `- **Outbound Promotion:** Pitch the v0.dev script draft to designers on LinkedIn. Copy template in the app sidebar to start.\n\n`;
  md += `---\n*RepurposeAI YouTube Autopilot Engine v1.0.0*`;

  // Save report locally
  fs.writeFileSync(path.join(REPORT_DIR, 'daily_report.md'), md);
  
  // Save copy to user artifacts directory for direct chat layout visualization
  const artifactPath = path.join(ARTIFACT_DIR, 'daily_report.md');
  fs.writeFileSync(artifactPath, md);
  console.log(`[Daemon] Compiled daily brief to artifact: ${artifactPath}`);
}
...
async function daemonTick() {
  ...
  console.log('\n[Daemon] Ticking automation loop...');
  ...
  try {
    ...
    if (queuedProductIndex !== -1) {
      ...
      console.log(`[Daemon] Found queued product to publish: ${queuedProduct.name}`);
      ...
      if (uploadResult.success) {
        ...
        console.log(`[Daemon] Saved publication history log: ${pubPath}`);
      }
    } else {
      console.log('[Daemon] No queued products found. Searching for new trends...');
      queueNewTool(data);
    }
...
// Run loop
console.log('🤖 YouTube SaaS Review Autopilot Daemon started!');
console.log(`⏰ Daemon running in DEVELOPMENT mode. Tick loop active every ${TICK_INTERVAL / 1000} seconds.`);

// Run immediately on launch
daemonTick();

// Set interval
setInterval(daemonTick, TICK_INTERVAL);
